...
public class IcebreakerController : MonoBehaviour
{
    ...
    private IEnumerator EndGameSequence(bool isVictory)
    {
        ...
        StopMovement();
        if (lightController != null) lightController.StartTransition();
        ...
        if (isVictory)
        {
            coinsToAward = health * 2; // Награда только за победу
        ...
        // Сообщаем GameManager'у, сколько монет заработано
        if (gameManager != null)
        {
        ...
        if (coinRewardController != null && coinsToAward > 0)
        {
